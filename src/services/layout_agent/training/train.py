...
def train_old(env, model, optimizer, num_episodes=300, max_steps_per_episode=700):
    print(torch.cuda.is_available())  # 如果返回 True，表示 PyTorch 识别到了 GPU
    print(torch.cuda.current_device())  # 显示当前使用的 GPU ID
...
def train(env, model, optimizer, num_episodes=300, max_steps_per_episode=700):
    # 初始化 TensorBoard 写入器
