...
class LayoutParser:
    ...
    def _parse_map(self, item: Dict) -> str:
        """解析地图"""
        return f"""
        <div >
            <div
                id="map" style="width: 100%; height: calc(100% - 60px); border: 1px solid #ccc; border-radius: 6px;"
            /div>
        </div>
        """

class LayoutParser_old:
    """布局解析器"""
    ...
    def _parse_content(self, content: List) -> str:
        ...
        for item in content:
            ...
            elif content_type in ['map']:
                html_parts.append(self._parse_map(map))
